...
int main(void) {
    ...
    for (k=0; k<300; k++){
        ...
        for (i = 0; i < NOMBRE_JOURS-1; i++) {
            if ((fam_jour[i+1] > fam_jour[i]) && ((fam_jour[i+1] - fam_jour[i]) > diff)){
                diff = (fam_jour[i+1] - fam_jour[i]);
                max_loc = fam_jour[i+1];
                indice_max = i+1;
                min_loc = fam_jour[i];
                indice_min = i;
            }
            if ((fam_jour[i+1] < fam_jour[i]) && ((fam_jour[i] - fam_jour[i+1]) > diff)){
                diff = (fam_jour[i] - fam_jour[i+1]);
                max_loc = fam_jour[i];
                indice_max = i;
                min_loc = fam_jour[i+1];
                indice_min = i+1;
            }
        }
